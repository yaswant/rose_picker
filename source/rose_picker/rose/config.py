...
.. testsetup:: *
    ...
    import os
    from rose_picker.rose.config import *

.. testcleanup:: rose_picker.rose.config
...
Classes:
    .. autosummary::
        rose_picker.rose.config.ConfigNode
        rose_picker.rose.config.ConfigNodeDiff
        rose_picker.rose.config.ConfigDumper
        rose_picker.rose.config.ConfigLoader

Functions:
    .. autosummary::
       rose_picker.rose.config.load
       rose_picker.rose.config.dump
...
import os.path
import re
from rose_picker.rose.env import env_var_escape
import shlex
import sys
from tempfile import NamedTemporaryFile, SpooledTemporaryFile
from functools import cmp_to_key
from rose_picker.rose.unicode_utils import write_safely
...
class ConfigNode:
    ...
    STATE_SYST_IGNORED = "!!"
    """ConfigNode state if a metadata operation has logically ignored the
    config."""
    ...
    def add(self, config_diff):
        ...
        for added_key, added_data in config_diff.get_added():
            ...
            self.set(keys=added_key, value=value, state=state, comments=comments)
    ...
    def dump(
        ...
        if root_option_keys:
            ...
            for key in root_option_keys:
                self._string_node_dump(key, root.value[key], handle, env_escape_ok)
...
class ConfigLoader:
    ...
    RE_SECTION = re.compile(r"^(?P<head>\s*\[(?P<state>!?!?))(?P<section>.*)\]\s*$")
    ...
    def load_with_opts(
        ...
        Examples:
            .. testcleanup:: rose_picker.rose.config.ConfigLoader.load_with_opts
    ...
    def _check_section_value(cls, section):
        ...
        for sym_open, sym_close in ["{}", "()"]:
            if index_of[sym_close] == -1 and index_of[sym_open] == -1:
                ...
                return len(section)
            elif index_of[sym_open] == -1 or index_of[sym_close] < index_of[sym_open]:
...
def dump(
    ...
    return ConfigDumper()(root, target, sort_sections, sort_option_items, env_escape_ok)
